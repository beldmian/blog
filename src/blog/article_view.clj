...
(defn render-tags [tags]
  (when (seq tags)
    [:div {:style {:margin-top (responsive/spacing :md)
     ...
     [:div {:style {:display "flex"
      ...
      (for [tag tags]
        [:span {:style {:background-color colors/surface
...
(defn article-page [data]
  (let [article (article-page-meta data)]
    [:div
     (render-tags (:tags article))
     (MarkdownRender (:contents article))]))
